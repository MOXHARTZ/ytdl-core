...
describe('【@ybd-project/ytdl-core】機能テスト', () => {
    describe('GetInfo 関数テスト', () => {
        ...
        describe('download', () => {
            const { normal } = VIDEO_IDS;

            it(`Download id: ${normal}`, async () => {
                const RESULTS = await ytdl_default.download('https://www.youtube.com/watch?v=' + normal);
                expect(RESULTS).toBeInstanceOf(ReadableStream);
            });
        })
        // describe('デフォルト', () => {
